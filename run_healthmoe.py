...
class DataProcessor(object):
    def get_examples(self, data_dir, dataset=None):
        ...
        csvdata = self._read_csv(os.path.join(data_dir, "{}.csv".format(dataset)))
        csvdata = csvdata[:int(len(csvdata)*0.1)] #테스트로 10%만 돌리기
        return self._create_examples(csvdata)
...
def get_priori(obj, response, context, evidence, T = 1):
    ...
    try:
        ...
        res_ctx_score = (1 - obj.cal_consine_similarities(response, context))
        res_evi_score = (1 - obj.cal_consine_similarities(response, evidence))
    except:
        res_ctx_score = 0
        res_evi_score = 0
...
def run_train(device, processor, tokenizer, model, writer, phase="train"):
    ...
    '''
    ep: 현재 에폭의 번호 (0부터 시작)
    step: 현재 배치(batch)의 번호 (0부터 시작)
    batch: 현재 배치(batch)에 해당하는 학습 데이터의 묶음입니다.
    따라서 이 중첩 for문은 총 100 x (10000/batch_size) 번 반복됩니다. 
    예를 들어, 만약 배치 크기가 32이고 학습 데이터셋의 크기가 10000이라면 총 3125번의 스텝(step)이 반복됩니다.
    
    각 스텝에서는 다음과 같은 학습 과정이 수행됩니다.
    batch에는 입력 텍스트 데이터와 정답 레이블이 포함됩니다.
    model에 batch를 입력으로 넣어 예측값 outputs를 계산합니다.
    outputs와 batch.labels를 사용하여 손실(loss)을 계산합니다.
    loss를 사용하여 backward()를 호출하여 모델의 파라미터에 대한 기울기(gradient)를 계산합니다.
    optimizer.step()을 호출하여 파라미터를 업데이트합니다.
    optimizer.zero_grad()을 호출하여 모든 파라미터에 대한 기울기를 0으로 초기화합니다.
    '''
    for ep in trange(args.num_train_epochs, desc="Training"):
        for step, batch in tqdm(enumerate(tr_dataloader)):
            ...
            loss.backward() #현재의 모델 파라미터에 대한 손실(loss)의 기울기(gradient)를 계산 -> 파라미터 업데이트시 활용
            del loss

            '''
            gradient_accumulation_steps는 gradient를 계산하는 스텝을 누적할 수 있는 값을 나타냅니다. 
            예를 들어, 만약 gradient_accumulation_steps=2로 설정했다면, 2개의 배치를 처리한 뒤 gradient를 계산하여 파라미터를 업데이트합니다.
            '''
            if (step + 1) % args.gradient_accumulation_steps == 0:  # optimizer를 사용하여 파라미터를 업데이트
                optimizer.step()
                optimizer.zero_grad()
                global_step += 1
            '''
            위 코드에서는 gradient_accumulation_steps 값이 설정된 만큼 배치를 처리한 후 optimizer.step()을 통해 파라미터를 업데이트하고, 
            optimizer.zero_grad()를 통해 gradient를 초기화합니다. 이후 global_step을 1 증가시킵니다. 이러한 과정을 통해 학습을 진행
            '''
            ...
            if args.do_eval and (((step + 1) % args.gradient_accumulation_steps == 0 and global_step % args.period == 0) or (ep==0 and step==0)):
                ...
                #logger.info('[DAN] >>>>>>>>>> {:.4}, {:.4}, {:.4} in run_train()'.format(dev_acc, dev_recall, dev_recall))
...
def run_eval(device, processor, tokenizer, model, writer, global_step, tensorboard=False,
    ...
    #logger.info('[DAN] >>>>>>>>>> result in run_eval()')
    #logger.info(result)
...
def main():
    ...
    # TensorboardX를 위한 SummaryWriter 생성
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", help="path to data", default='./healthver/data')
    ...
    #parser.add_argument("--train_batch_size", default=1000) #quick run!
    parser.add_argument("--eval_batch_size", default=32)
    #parser.add_argument("--eval_batch_size", default=1000) #quick run!
    parser.add_argument('--debug_mode', action='store_true')
    parser.add_argument("--learning_rate", default=2e-5, type=float, help="The initial learning rate for Adam.")
    #parser.add_argument("--num_train_epochs", default=20) #original
    parser.add_argument("--num_train_epochs", default=8) #quick run!
